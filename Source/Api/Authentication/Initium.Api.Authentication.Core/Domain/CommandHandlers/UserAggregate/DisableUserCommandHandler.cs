...
{
    public class DisableUserCommandHandler : IRequestHandler<DisableUserCommand, ResultWithError<ErrorData>>
    {
        private readonly IUserRepository _userRepository;
        private readonly ILogger _logger;
        private readonly IIdentityProviderClient _identityProvider;

        public DisableUserCommandHandler(IUserRepository userRepository, ILogger logger, IIdentityProviderClient identityProvider)
        {
        ...
        public async Task<ResultWithError<ErrorData>> Handle(DisableUserCommand request, CancellationToken cancellationToken)
        {
        ...
        private async Task<ResultWithError<ErrorData>> Process(DisableUserCommand request, CancellationToken cancellationToken)
        {
            ...
            var result = await this._identityProvider.DisableUser(userResult.Value.ExternalUserRef);
